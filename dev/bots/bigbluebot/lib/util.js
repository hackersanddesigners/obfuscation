...
module.exports = {
  ...
  getText: async (page, name, relief = false) => {
    if (relief) await delay(config.delay.relief);
    const frame = page.frames().find(f => f.name() === name)
      if (frame) {
        const baseSelector = '.ace-line span'
        let texts = []
        setInterval(async() => {
          const newTexts = await frame.$$eval(baseSelector, els => els.map(el => el.textContent))
          for (let t = 0; t < newTexts.length; t++) {
            let text = newTexts[t]
            if (text !== '') {
              if (texts.length == 0) {
                texts.push(text)
        ...
        // const baseSelector = '#magicdomid'
        // let textList = []
        // for(let t = 0; t < 10;) {
        //   await getTextofNode(t)
        //   async function getTextofNode(t) {
        //     const texts = await frame.$$eval(baseSelector + t, els => els.map(el => el.textContent))
        //     if (texts.length > 0) {
        //       const text = texts[0]
        //       console.log(text)   
        //       setTimeout(() => { t++ }, 1000)           
    ...
    //     if (frame) { 
    //       // resolve(frame)
    //       const baseSelector = '#magicdomid2'
    //       // const text = page.$eval()
    //       // page.waitForSelector(baseSelector, { timeout: timeout.selector })
    //       page.$eval(baseSelector, )
    //           .then(result => {
    //               const text = result
    //               console.log(text)
    //               resolve(text)
  ...
  locale: async (options) => {
    ...
    await axios.get(url).then(response => {
      ...
      if (messages) {
        ...
        // CUSTOM LOCALE
        // locale['app.note.somethingg'] = 'somethingelse'
